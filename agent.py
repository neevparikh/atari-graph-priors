...
class Agent():
    ...
    def act_e_greedy(self, state, epsilon=0.001):  # High ε can reduce evaluation scores drastically
        return np.random.randint(
            0, self.action_space) if np.random.random() < epsilon else self.act(state)
    ...
    def learn(self, mem):
        ...
        with torch.no_grad():
            ...
            Tz = returns.unsqueeze(
                1) + nonterminals * (self.discount**self.n) * self.support.unsqueeze(
        ...
        mem.update_priorities(
            idxs, loss.detach().cpu().numpy())  # Update priorities of sampled transitions
