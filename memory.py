...
class SegmentTree():
    ...
    def _retrieve(self, indices, values):
        ...
        successor_indices = children_indices[successor_choices, np.arange(
            indices.size)]  # Use classification to index into the indices matrix
...
class ReplayMemory():
    ...
    def _get_samples_from_segments(self, batch_size, p_total):
        ...
        while not valid:
            ...
            if np.all((self.transitions.index - idxs) % self.capacity > self.n) and np.all(
                (idxs - self.transitions.index) % self.capacity >= self.history) and np.all(
                    probs != 0):
        ...
        states = torch.tensor(all_states[:, :self.history], device=self.device,
        ...
        nonterminals = torch.tensor(np.expand_dims(transitions['nonterminal'][:,
                                                                              self.history +
                                                                              self.n - 1],
                                                   axis=1),
                                    dtype=torch.float32,
    ...
    def sample(self, batch_size):
        ...
        weights = torch.tensor(
            weights / weights.max(), dtype=torch.float32,
